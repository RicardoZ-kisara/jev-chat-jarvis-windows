...
5. 在工作台选择已导入会话并点击「分析可能性并生成回复」，或在长期档案点击「分析此会话并给出回复」。直接使用近期记录与相关历史，无需先生成完整档案；此时才调用 ChatGPT 并消耗账号额度。
6. 如需整理全部长期记录，再查看完整档案范围并生成档案。后续回复会使用有效总档案、相关原文和分段记忆。详见 [分析与回复](ANALYSIS.md)。
