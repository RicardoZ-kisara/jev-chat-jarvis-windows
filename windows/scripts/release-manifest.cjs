...
const shipped = [...walk(path.join(root,'src')), ...walk(path.join(root,'native')), path.join(root,'resources/icon.ico')];
...
const binaries=fs.readdirSync(path.join(root,'dist')).filter(name=>name.startsWith('Jev-QQ-Windows-')&&name.endsWith('.exe')).map(name=>({name,size:fs.statSync(path.join(root,'dist',name)).size,sha256:digest(path.join(root,'dist',name))}));
