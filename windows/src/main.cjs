...
const {DEFAULTS, validateSettings, analyze} = require('./core.cjs');
...
app.whenReady().then(async () => {
  ...
  handle('analysis:run', async input => {
    if (controller || summaryController) throw new Error('正在分析，请等待或暂停当前任务。');
  ...
  handle('history:import', async input => {
    if (summaryController) throw new Error('请先暂停档案生成，再导入新记录。');
  ...
  handle('history:profile', id => history.profile(String(id)));
  handle('history:plan', id => {const item=history.list().find(s=>s.id===id);if(!item)throw new Error('请选择历史会话。');const rows=history.rows(id);return {...item,batches:chunks(rows).length,characters:rows.reduce((n,r)=>n+r.text.length,0),endpoint:new URL(settings.replyUrl).origin,model:settings.replyModel};});
  handle('history:summarize', async id => {
    if(summaryController||controller)throw new Error('已有分析任务进行中，请等待或暂停。');
  ...
  handle('history:delete', async id => {
    if(summaryController)throw new Error('请先暂停档案生成。');
...
app.on('window-all-closed', () => {controller?.abort(); summaryController?.abort(); worker?.terminate(); app.quit();});
